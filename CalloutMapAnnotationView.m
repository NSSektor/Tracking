...
- (void)prepareFrameSize {
    ...
    CGFloat width = self.contentWidth; //moddified by kv1 
    
	
	//frame.size = CGSizeMake(self.mapView.frame.size.width, height); //original code
    frame.size = CGSizeMake(width, height); //moddified by kv1
...
- (void)prepareOffset { //offest of the whole box
    ...
    CGFloat x = parentOrigin.x + (320.f / 2); //moddified by kv1 self.contentWidth why not?! 
   ...
   self.centerOffset = CGPointMake(0.f, yOffset);
...
- (void)adjustMapRegionIfNeeded {
	...
	if ([self relativeParentXPosition] < temp) {
		xPixelShift = temp - [self relativeParentXPosition];
        
        //NSLog(@"%f", [self relativeParentXPosition]);
        
        
	} else if ([self relativeParentXPosition] > self.frame.size.width - temp) {
		xPixelShift = (self.frame.size.width - temp) - [self relativeParentXPosition];
	}
...
- (void)drawRect:(CGRect)rect {
    ...
    if( self.centerOffset.x == 0.0f )
        parentX = self.contentWidth / 2; // moddified by kv1
    else if( self.centerOffset.x < 0.0f )
        parentX = (self.contentWidth / 2) + -self.centerOffset.x; // moddified by kv1
